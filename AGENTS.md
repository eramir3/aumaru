...
- Product nutrition image names depend on product slugs. The template shows available `-fr.webp` and `-en.webp` files and omits missing images.
...
1. Install Node dependencies with `npm install` if needed. Start the local stack with `make docker-up`; the site uses `http://localhost:8080/` by default. Run `make` for other targets. `make docker-reset` preserves data volumes; `make docker-reset-data` deletes them.
...
3. If PHP files change, run `php -l` on the changed files. If Tailwind classes or configuration change, regenerate the tracked CSS with `npx tailwindcss -i ./src/input.css -o ./src/output.css` (`make tailwind` runs the npm watcher).
